...
```{r randomise, include = TRUE, eval = FALSE}
set.seed(1111)
...
### Light intensity data and analyses.   
...
Growth data as weekly leaf counts (Day 0 - Day 49) and leaf area mesured from photos taken on Day 28.
...
Note: code chunks for actual processing of the images are at the end of this document - they are not run or knitted, as this take long time and the analyses below use processed and saved output.    
...
Figure 2 for the manuscript (not including here - saving into a file).   
...
kable(
  ...
  caption = "S1. Results of Wilcoxon runk sum tests to cempare medians of two groups."
  )
```
...
As one table for the main text (not shown here).    
...
## Differences between groups - plots 

Figure 3 for the manuscript (not including here - saving into a file).   

```{r plot differences between groups, include = TRUE, eval = FALSE}

### Dead leaves on Day 42 and Day 49:
...
Figure 4 for the manuscript (not including here - saving into a file).    
...
```{r leaf counts correlations tests, include = TRUE, eval = TRUE}
dat_image %>%
  filter(!is.na(day28_leaves)) %>%
  filter(!is.na(day28_leaf_area)) %>%
  group_by(group) %>%
  summarize(cor(day28_leaves, day28_leaf_area)) 
  #summarize(test = cor.test(day28_leaves, day28_leaf_area)$p.value) #for p-values

dat_image %>%
  filter(!is.na(day49_leaves)) %>%
  filter(!is.na(day49_prop_dark_area)) %>%
  group_by(group) %>%
  summarize(cor(day49_leaves, day49_prop_dark_area)) 
  #summarize(test = cor.test(day49_leaves, day49_prop_dark_area)$p.value) #for p-values
```

```{r leaf counts correlations plots, include = FALSE, eval = FALSE}

#plot correlation for Day 48
corr28 <- dat_image %>% 
  filter(!is.na(day28_leaves)) %>%
  filter(!is.na(day28_leaf_area)) %>%
  ...
  ggpubr::stat_cor(aes(color = group), p.accuracy = 0.001, label.x.npc = "centre", label.y.npc = "top")  #add correlation coefficients to the plot

#plot correlation for Day 49
corr49 <- dat_image %>% 
  filter(!is.na(day49_leaves)) %>%
  filter(!is.na(day49_prop_dark_area)) %>%
...
#save the figure 
ggsave(here("plots", "plot.groupcorrs.png"), grid.arrange(corr28, corr49, ncol = 2, nrow = 1), width = 16, height = 8, units = "cm", dpi = 300, scale = 1.4)      
```




### Load images and trim [do not run]
...
```{r plot two panels for poster data, include = FALSE, eval = FALSE}
## One plot with two panels of differences between the groups for the poster
# using library(ggplot2) and library(gridExtra)

plot1 <- ggplot(dat_image, aes(x = group, y = day49_leaves)) + 
  ...
  labs(title = "Figure 1. Duckweed Growth as Leaf Count") +
  scale_fill_manual(values = c("darkgreen", "lightblue2")) +
  labs(y = "Leaf count")

plot2 <- ggplot(dat_image, aes(x = group, y = prop_dark_area)) + 
  geom_boxplot() + 
  theme_minimal() +
  theme(legend.position = "none") +
  theme(plot.title = element_text(hjust = 0.5),
    ...
    color = "green4",
    size = 0.5, linetype = "solid")) +
  labs(title = "Figure 2. Duckweed Leaf Underside Pigmentation") +
  scale_fill_manual(values = c("darkgreen", "lightblue2")) +
  labs(y = "Proportion of dark pigmented area") +
  geom_jitter(shape=16, position=position_jitter(0.2), alpha = 0.5)

ggsave("figs_poster.png", grid.arrange(plot1, plot2, ncol = 2), width = 24, height = 10, unit = "cm")
```
