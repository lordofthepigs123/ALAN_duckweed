...
ggplot(dat_image, aes(x = group, y = prop_dark_area)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5),
    ...
    color = "green4",
    size = 0.5, linetype = "solid")) +
  labs(title = "Duckweed Pigmentation") +
  scale_fill_manual(values = c("darkgreen", "lightblue2")) +
  labs(y = "proportion of dark under the leaf") +
  geom_jitter(shape=16, position=position_jitter(0.2), alpha = 0.5)

ggsave("fig_pigmentation.png", width = 10, height = 8, unit = "cm")
...
dat_image$group <- as.factor(dat_image$group)
levels(dat_image$group)
levels(dat_image$group) <- c("ALAN", "Control")
...
ggplot(dat_image, aes(x = group, y = day49_leaves)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5),
    ...
    color = "green4",
    size = 0.5, linetype = "solid")) +
  labs(title = "Duckweed Growth Leaf Count") +
  scale_fill_manual(values = c("darkgreen", "lightblue2")) +
  labs(y = "Leaf Count")

ggsave("fig_growth.png", width = 10, height = 8, unit = "cm")

```


## Two plots of difference between the groups for the poster
